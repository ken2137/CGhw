...
function initHUD() {
  ...
  let planeTB = new THREE.Mesh( new THREE.PlaneGeometry( 0.5, 50 ), new THREE.MeshBasicMaterial( {color: 0xc0c0c0, side: THREE.DoubleSide} ));
  planeTB.position.set(10.5, 0, 0);
  sceneHUD.add( planeTB );
  planeTB = planeTB.clone();
  planeTB.position.set(-10.5, 0, 0);
  sceneHUD.add( planeTB );
  let planeLR = new THREE.Mesh( new THREE.PlaneGeometry( 50, 0.5 ), new THREE.MeshBasicMaterial( {color:0xc0c0c0, side: THREE.DoubleSide} ));
...
function init() {
	...
	let directionalLight = new THREE.DirectionalLight(0x11111);
	directionalLight.position.set(0, 0, 1).normalize();
	scene.add(directionalLight);

	let pointLight = new THREE.PointLight(0xc0c0c0);
...
function Buildcar(){
	...
	car.add(body,wheel,wheel2,wheel3,wheel4);
	return car;
}
